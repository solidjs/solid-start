...
import { addTodoMutation, fetchTodos, removeTodoMutation, updateTodoMutation } from "../fake-api";

import { action, createLoader, useAction, useSubmission, useSubmissions } from "../data";
              ...
              <Show when={!removingTodo.pending}>
                <div>
                  <input
                    ...
                    onChange={e => submitUpdate({ id, done: e.currentTarget.checked })}
                  />
                  <input
                    type="text"
                    value={title}
                    disabled={updatingTodo.pending}
                    onChange={e => submitUpdate({ id, title: e.currentTarget.value })}
                  />
                  <button disabled={updatingTodo.pending} onClick={[submitRemove, id]}>
                    x
                  </button>
                </div>
              </Show>
