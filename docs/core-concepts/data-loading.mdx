...
Since we are full-stack framework, and we want to make it easy to write data loading code that's meant to run on the server only. It could be database access, or internal APIs, etc. We created `createServerData$` for this. It builds upon our other primitives like `createRouteData` and `server$` to give a generated RPC for your data.
