...
```html
<form method="post" action="/what-url-can-we-put-here?">
...
We want to handle the request sent by the form here. Normally, if you want to handle the form submission client side you would overwrite the onSubmit
callback on the form element and handle the data from the submission yourself. But this has a few fallbacks:
...
```tsx filename=app/routes/projects

function createProject(form: FormData) {
  const project = await createProject(form);
  redirect(`/projects/${project.id}`);
}
const createProject = createAction(server((form) => {
