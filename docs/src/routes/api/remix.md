...
Remix uses the browser's cache for prefetching with HTML `< link rel="prefetch"/ >`tags, which provides a lot subtle benefits (like respecting HTTP cache headers, doing the work in browser idle time, using a different thread than your app, etc.) but the implementation might mess with your CSS since the link tags are rendered inside of your anchor tag. This means`a _:last-child {}`style selectors won't work. You'll need to change them to`a _:last-of-type {}` and you should be good. We will eventually get rid of this limitation.
...
function PendingLink({ to, children }) {
  ...
  return <Link data-pending={isPending ? "true" : null} to={to} children={children} />;
}
```
...
export default function NewsletterSignupRoute() {
  ...
  return (
    <Form method="post" action="/newsletter/subscribe">
      <p>
        <input type="text" name="email" /> <button type="submit">Subscribe</button>
      </p>
  ...
  return (
    <cities.Form method="get" action="/city-search">
      <Combobox aria-label="Cities">
        <div>
          <ComboboxInput name="city-query" onChange={event => cities.submit(event.target.form)} />
...
function Task({ task }) {
  ...
  return (
    <toggle.Form method="put" action={`/project/${projectId}/tasks/${id}`}>
      <label>
        <input type="checkbox" checked={checked} onChange={e => toggle.submit(e.target.form)} />
      </label>
    </toggle.Form>
  );
}
...
function ProjectTaskCount({ project }) {
  ...
  for (const f of fetchers) {
    if (f.submission && f.submission.action.startsWith(`/projects/${project.id}/task`)) {
      const taskId = f.submission.formData.get("id");
      myFetchers.set(parseInt(taskId), f.submission.formData.get("complete") === "on");
    }
  }
...
export let action: ActionFunction = async ({ request }) => {
  ...
  let uploadHandler: UploadHandler = async ({ name, stream }) => {
    ...
    const uploadedImage = await cloudinary.v2.uploader.upload(stream, {
      public_id: userId,
...
export let cloudinaryUploadHandler = createCloudinaryUploadHandler({
  folder: "/my-site/avatars"
});
...
export default function CompaniesRoute() {
  ...
  const selectedCompanyIndex = data.companies.findIndex(company => company.id === params.companyId);
      ...
      <Accordion index={selectedCompanyIndex}>
        {data.companies.map(company => (
          <AccordionItem key={company.id}>
            ...
            <AccordionPanel>{params.companyId === company.id ? outlet : null}</AccordionPanel>
          </AccordionItem>
        ))}
      </Accordion>
    </div>
...
export default function CompanyRoute() {
  ...
  const sortedInvoices = sort === "ASC" ? data.company.invoices : data.company.invoices.reverse();
