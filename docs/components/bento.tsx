...
export const BentoItem = (props: BentoItemProps) => {
  return (
    ...
    >
      <em
        class={`not-italic text-lg pb-3 block ${getAccent(props.accent).title}`}
      >
        {props.title}
      </em>
      <p>{props.children}</p>
...
export const BentoBox = (props: { children: JSX.Element }) => {
  return (
    <ul class="max-w-full w-full px-4 flex flex-col md:grid grid-cols-1 sm:grid-cols-2 grid-rows-1 sm:grid-rows-2 gap-5 sm:gap-4">
      {props.children}
    </ul>
  );
};
