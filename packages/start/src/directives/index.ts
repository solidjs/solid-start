...
const CLIENT_PATH = normalizePath(
  fileURLToPath(new URL("../runtime/server-runtime.js", import.meta.url)),
);
const SERVER_PATH = normalizePath(
  fileURLToPath(new URL("../runtime/server-fns-runtime.js", import.meta.url)),
);
...
function invalidateModules(
  ...
  if (server && result.invalidPreload) {
    ...
    },
    {
      ...
      async load(id, opts) {
        ...
        if (id === options.manifest) {
          ...
          return await current.promise.reference;
        }
        return null;
      },
    },
