...
export function isEvent(obj: any): obj is H3Event | { [HTTPEventSymbol]: H3Event } {
  return (
    typeof obj === "object" &&
...
type WrapFunction<TFn extends (...args: Array<any>) => any> = PrependOverload<
  TFn,
  (
    ...args: Parameters<TFn> extends [H3Event, ...infer TArgs] ? TArgs : Parameters<TFn>
  ) => ReturnType<TFn>
>;
...
function createWrapperFunction<TFn extends (...args: Array<any>) => any>(
  ...
  return function (...args: Array<any>) {
    ...
    if (!isEvent(event)) {
      ...
      args[0] =
        event instanceof H3Event || (event as any).__is_event__ ? event : event[HTTPEventSymbol];
    }
...
export const readRawBody: PrependOverload<typeof _readRawBody, WrappedReadRawBody> =
...
type WrappedGetValidatedQuery = <T, TEventInput = InferEventInput<"query", H3Event, T>>(
...
>(
  ...args: Tail<Parameters<typeof _getValidatedRouterParams<T, H3Event, TEventInput>>>
...
export const getResponseStatusText = createWrapperFunction(_getResponseStatusText);
...
export const appendResponseHeaders = createWrapperFunction(_appendResponseHeaders);
...
export const fetchWithEvent: PrependOverload<typeof _fetchWithEvent, WrappedFetchWithEvent> =
  createWrapperFunction(_fetchWithEvent);
export const getProxyRequestHeaders = createWrapperFunction(_getProxyRequestHeaders);
...
export const updateSession: PrependOverload<typeof _updateSession, WrappedUpdateSession> =
...
export const sealSession = createWrapperFunction(_sealSession) as PrependOverload<
  typeof _sealSession,
  WrappedSealSession
>;
...
export const appendCorsPreflightHeaders = createWrapperFunction(_appendCorsPreflightHeaders);
...
export const appendHeader: PrependOverload<typeof _appendHeader, WrappedAppendHeader> =
...
export const getRequestFingerprint = createWrapperFunction(_getRequestFingerprint);
...
export const readMultipartFormData = createWrapperFunction(_readMultipartFormData);
type WrappedReadValidatedBody = <T, TEventInput = InferEventInput<"body", H3Event, T>>(
...
export type RequestHandler = (ctx: { request: Request }) => Promise<Response> | Response;
